...
my $template = load_file('./template.tex');
...
my $cycle_list;
if ($label =~ /gtfs/i) {
  @steps = qw(fix xml sparql opt. map~step);
  $cycle_list = '%
{blue,fill=blue!30!white,mark=none},
{red,fill=red!30!white,mark=none},
{brown!60!black,fill=brown!30!white,mark=none},
{black,fill=gray!30!white,mark=none},
{black,fill=white,mark=none},';
} elsif ($label =~ /empty/i) {
  @steps = qw(fix null sparql opt. map~step);
  $cycle_list = '%
{blue,fill=blue!30!white,mark=none},
...
{black,fill=white,mark=none},';
  ...
  @steps = qw(fix sparql opt. map~step);
  $cycle_list = '%
{blue,fill=blue!30!white,mark=none},
{brown!60!black,fill=brown!30!white,mark=none},
{black,fill=gray!30!white,mark=none},
...
for my $xcol (@columns) {
  ...
  my $tf_fn = "${prefix}${xcol}/triples_count.txt";
  open my $tf, '<', $tf_fn or die "$tf_fn: $!";
  my ($triples) = split ' ', <$tf>;
  $data{$col}{triples} = $triples;
